...
define([
	...
	Point.prototype = {
		...
		updateTime: function (media) {
			...
			if (this.time[0] === this.time[1]) {
				this.start = this.end = Media.secondsTo(media.time(this.time[0]), 'ms');
			} else {
				this.start = Media.secondsTo(media.time(this.time[0]), 'ms');
				this.end = Media.secondsTo(media.time(this.time[1]), 'ms');
			}
	...
	Media.extend({
		...
		createTimeline: function (name, options) {
			...
			if (!this.timeline || !this.timelines) {
				...
				this.on('play seeked', function () {
					...
					if (length) {
						var ms = Media.secondsTo(this.time(), 'ms'), k;
			...
			this.timelines[name] = new Timeline(options.data);
		...
		timelineTimeout: function () {
			...
			var ms = Media.secondsTo(this.time(), 'ms');
			var total_ms = Media.secondsTo(this.duration(), 'ms');
			var point, k, length;
